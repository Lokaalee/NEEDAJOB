...
if (!isset($_SESSION['user_id'])) {
    ...
    header("Location: login.php?redirect=apply_job.php?job_id=" . $job_id); 
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['resume']) && $_FILES['resume']['error'] == 0) {
        ...
        if (!in_array($file_type, $allowed_types)) {
            echo '<div class="message error">Only PDF or DOCX files are allowed.</div>';
            exit;
        }

        // Validate file size (max 2MB)
        if ($_FILES['resume']['size'] > 2 * 1024 * 1024) {
            echo '<div class="message error">File size must be less than 2MB.</div>';
            exit;
        }

        // Sanitize file name and store it
        $resume_filename = basename($_FILES['resume']['name']);
        $resume_path = 'C:/xampp/htdocs/NEEDAJOB/uploads/' . preg_replace("/[^a-zA-Z0-9._-]/", "_", basename($_FILES['resume']['name']));
        ...
        if (move_uploaded_file($_FILES['resume']['tmp_name'], $resume_path)) {
            ...
            $stmt_apply->bind_param("iisss", $job_id, $user_id, $job['job_title'], $job['company'], $resume_path);
            ...
            echo '<div class="message error">Failed to upload resume. Please try again.</div>';
        }
    } else {
